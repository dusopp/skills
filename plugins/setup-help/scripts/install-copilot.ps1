# setup-help install-copilot.ps1
# Makes the setup-help skill available to GitHub Copilot (Copilot CLI + VS Code agent
# mode) by linking it into the user-level Copilot skills directory, which both read:
#   %USERPROFILE%\.copilot\skills\setup-help  ->  <this plugin>\skills\setup-help
#
...
$target = Join-Path $TargetRoot 'setup-help'
...
$source = Join-Path (Split-Path -Parent $PSScriptRoot) 'skills\setup-help'
