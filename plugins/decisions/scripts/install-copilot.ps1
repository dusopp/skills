# decisions install-copilot.ps1
# Makes the decisions skill available to GitHub Copilot (Copilot CLI + VS Code agent
# mode) by linking it into the user-level Copilot skills directory, which both read:
#   %USERPROFILE%\.copilot\skills\decisions  ->  <this plugin>\skills\decisions
#
...
$target = Join-Path $TargetRoot 'decisions'
...
$source = Join-Path (Split-Path -Parent $PSScriptRoot) 'skills\decisions'
